...
<template>
  <div class="relative flex flex-col gap-1">
    ...
    <template v-else>
      ...
      <component :is="icon" v-if="icon" class="absolute left-6 top-[calc(50%_+_3px)] h-5 w-5 -translate-x-1/2 -translate-y-1/2 text-gray-400" />
      <input class="flex h-14 flex-col items-start justify-center self-stretch rounded-lg border border-gray-300 py-4 pr-6 text-gray-700 transition-colors duration-200 ease-in-out placeholder:text-gray-400 hover:transition-all hover:duration-200 hover:ease-in-out focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-600 focus:transition-all focus:duration-200 focus:ease-in-out active:ring-2 active:ring-blue-600" v-bind="$attrs" :class="[icon ? 'pl-12' : 'pl-6']">
