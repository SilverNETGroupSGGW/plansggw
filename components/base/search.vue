...
const filteredOptions = computed(() => {
  return props.options.filter((option) => {
    ...
    return option.value.toLowerCase().startsWith(query.value.toLowerCase())
  }).slice(0, 3)
})
</script>
  ...
  <div class="relative flex flex-col">
    <Combobox :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)">
      ...
      <div class="relative h-full">
        ...
        <ComboboxInput class="flex h-12 w-full flex-col items-start justify-center self-stretch rounded-lg border border-gray-300 py-4 pr-6 text-gray-700 transition-colors duration-200 ease-in-out placeholder:text-gray-400 hover:transition-all hover:duration-200 hover:ease-in-out focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-600 focus:transition-all focus:duration-200 focus:ease-in-out active:ring-2 active:ring-blue-600 disabled:bg-gray-100" v-bind="$attrs" :class="[icon ? 'pl-12' : 'pl-6']" @change="query = $event.target.value" />

        <TransitionRoot leave="transition ease-in duration-100" leave-from="opacity-100" leave-to="opacity-0" @after-leave="query = ''">
          <div v-if="!filteredOptions.length" class="absolute z-10 mt-4 max-h-60 w-full overflow-auto rounded-lg border border-gray-200 bg-white px-1 py-2 text-base shadow-lg focus:outline-none">
