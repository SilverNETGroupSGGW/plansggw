...
<template>
  ...
  <div>
    <TabGroup @change="tabIndex = $event">
      <TabList class="flex gap-8 w-full border-b border-gray-200 px-12">
        <Tab v-for="(day, index) in daysOfWeek" :key="index" v-slot="{ selected }" as="template">
          <base-button variant="flat" class="rounded-none py-4 bg-white" :class="[selected && 'border-b-2 border-b-blue-600']">
            {{ day.label }}
          </base-button>
        </Tab>
      </TabList>

      <TabPanels class="h-[calc(100vh-4rem)] select-none overflow-scroll">
        <TabPanel v-for="(day) in daysOfWeek" :key="day.label">
